...
LLC_FLAGS="-regalloc=chaitin_briggs -march=mips"
for f in $(ls *.c); do
  b=${f%.c}
  /class/cs426/mips_linux_toolchain/bin/clang -I/class/cs426/mips_linux_toolchain/lib/gcc/mips-linux-gnu/4.4.6/include -I/class/cs426/mips_linux_toolchain/lib/gcc/mips-linux-gnu/4.4.6/include-fixed -I/class/cs426/mips_linux_toolchain/mips-linux-gnu/libc/usr/include -m32 -ccc-host-triple mips-gnu-linux -ccc-clang-archs mips -Wimplicit-int -msoft-float -std=gnu89 -D_MIPS_SZPTR=32 -D_MIPS_SIM=_ABIO32 -emit-llvm -c $b.c -o $b.bc
